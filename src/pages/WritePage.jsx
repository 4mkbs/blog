...
export default function WritePage() {
  ...
  const [blocks, setBlocks] = useState([{ type: "paragraph", data: { text: "" } }]);
  ...
  useEffect(() => {
    ...
    const loadPost = async () => {
      try {
        ...
        const post = res.data;
        setTitle(post.title || "");
        setBlocks(post.content?.length ? post.content : [{ type: "paragraph", data: { text: "" } }]);
  ...
  const handlePublish = useCallback(async (publishStatus) => {
    if (!title.trim()) {
        ...
        <div className="write-cover-section">
          {coverImage ? (
            <div className="write-cover-preview">
              ...
              <button onClick={() => setCoverImage("")} className="write-cover-remove">✕</button>
            </div>
          ) : (
            <input
              type="url"
